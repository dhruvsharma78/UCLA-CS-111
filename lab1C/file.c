...
int closeFile(simpsh_filetable_t* ft, int fd){
  if(fd < ft->num){
    ft->files[fd].status = 0;
    if(close(ft->files[fd].fd) < 0) return -1;
    return 0;
  }
  return -1;
}
...
int closeOtherFiles(int start, int end){
  int i;
  int err = 0;
  for(i=start; i<=end; ++i){
    if(close(i) < 0){
